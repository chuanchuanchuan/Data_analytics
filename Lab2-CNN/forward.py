...
def forward(input, train, regularizer=None):
    ...
    with tf.variable_scope('layer5-fc1',reuse=tf.AUTO_REUSE):
        fc1_weights = tf.get_variable(
            'weight', [nodes, FC_NUM], initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer6-fc2',reuse=tf.AUTO_REUSE):
        fc2_weights = tf.get_variable(
            "weight", [FC_NUM, LABELS], initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer != None:
